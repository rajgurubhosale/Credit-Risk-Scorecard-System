...
# Page config 
st.set_page_config(page_title="Credit Risk Predictor", page_icon="💳", layout="wide",initial_sidebar_state="collapsed",
)
hide_sidebar()
display_top_nav_bar()
...
if predict_btn:
    ...
    payload_features = {
        ...
        'employment_payment_history_feature':{
    ...
    #fastapi url port change later
    try:    
        response = requests.post(f'{BASE_FASTAPI_URL}/predict', json=payload_features,timeout=10)
        ...
        if response.status_code == 200:
            ...
            required_keys = ["score", "pd_value", "risk_label", "approval_decision", "feature_score_breakdown"]
            missing = [k for k in required_keys if k not in result]
            
            if missing:
                st.error(f"❌ Missing fields in response: {missing}")
                
            risk_label = result['risk_label']
            score = result['score']
            pd_value = result['pd_value']
            breakdown_features = result['feature_score_breakdown']
    ...
    with col_decision:
        if approval_decision == 'APPROVE':
            st.success("✅ APPROVE")
        elif approval_decision =='MANUAL REVIEW':
    ...
    with col_risk:
        ...
        elif risk_label == "Very High Risk":
